...
CALIBRATION="calibration_STAR_PHENIX_CuAu200GeV.yoda"
#Flags of your analysis (Ex. centrality: cent=GEN)
RIVET_FLAGS=":cent=GEN:beam=CUAU200"
rm /tmp/$FIFOFILE
mkfifo /tmp/$FIFOFILE
./main113 /tmp $FIFOFILE $NEVENTS $BEAM1 $BEAM2 $CMS_ENERGY $GENERATOR_SEED $PTHARDMIN $PTHARDMAX & rivet --pwd -p ../Centralities/Calibration/calibration_STAR_PHENIX_CuAu200GeV.yoda -a $ANALYSIS$RIVET_FLAGS -o Rivet.yoda /tmp/$FIFOFILE
