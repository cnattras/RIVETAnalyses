...
namespace Rivet {
  ...
  public:
    ...
    /// Constructor
    RIVET_DEFAULT_ANALYSIS_CTOR(PHENIX_2012_I1107625);
...
//create binShift function
    ...
    void init() {
      ...
      const Estimate1D& refdataRaa12 =refData(refnameRaa12);
    ...
    void analyze(const Event& event) {
      ...
      if (beam.first.pid() == 2212 && beam.second.pid() == 2212)
      {
          ...
          if (fuzzyEquals(sqrtS()/GeV, 62.4, 5)) collSys = pp62;
            ...
            Particles neutralParticles = apply<UnstableParticles>(event,"ufs").particles();
...
//RAA _______________________________
      ...
      hRaa["Raa_c010_AuAu39"]->scale(1./777.2);
      hRaa["Raa_c1020_AuAu39"]->scale(1./496.7);
      hRaa["Raa_c2040_AuAu39"]->scale(1./253.6);
      hRaa["Raa_c4060_AuAu39"]->scale(1./81.81);
      hRaa["Raa_c6086_AuAu39"]->scale(1./13.88);
      hRaa["Raa_minbias_AuAu39"]->scale(1./401.6);

      hRaa["Raa_c010_AuAu62"]->scale(1./843.0);
      hRaa["Raa_c1020_AuAu62"]->scale(1./535.8);
      hRaa["Raa_c2040_AuAu62"]->scale(1./270.5);
      hRaa["Raa_c4060_AuAu62"]->scale(1./85.71);
      hRaa["Raa_c6086_AuAu62"]->scale(1./14.29);
      hRaa["Raa_minbias_AuAu62"]->scale(1./426.9);
...
//Centrality vs RAA_______________________________
      ...
      for(auto element : hRaa)
      {
          ...
          for(auto &bin : yodaRaa.bins())
          {

              if(bin.xMid() > 4. && bin.xMid() < 6.)
              {
                  if(!isnan(bin.val()))
                  {
                      averageRaaPt46 += bin.val();
                      averageRaaPt46Error += sqr(bin.errAvg());
                      nbinsPt46++;
                  }
              ...
              if(bin.xMid() > 6.)
              {
                  if(!isnan(bin.val()))
                  {
                      averageRaaPt610 += bin.val();
                      averageRaaPt610Error += sqr(bin.errAvg());
                      nbinsPt610++;
                  }
              }
          }
          ...
          {
              hRaaNpart["Raa_pt46_AuAu39"]->bin(centBins[name]+1).setVal(averageRaaPt46/nbinsPt46);
              hRaaNpart["Raa_pt46_AuAu39"]->bin(centBins[name]+1).setErr(sqrt(averageRaaPt46Error)/nbinsPt46);
              hRaaNpart["Raa_pt610_AuAu39"]->bin(centBins[name]+1).setVal(averageRaaPt610/nbinsPt610);
              hRaaNpart["Raa_pt610_AuAu39"]->bin(centBins[name]+1).setErr(sqrt(averageRaaPt610Error)/nbinsPt610);
          }
          else if(name.find("62") != std::string::npos)
          {
              hRaaNpart["Raa_pt46_AuAu62"]->bin(centBins[name]+1).setVal(averageRaaPt46/nbinsPt46);
              hRaaNpart["Raa_pt46_AuAu62"]->bin(centBins[name]+1).setErr(sqrt(averageRaaPt46Error)/nbinsPt46);
              hRaaNpart["Raa_pt610_AuAu62"]->bin(centBins[name]+1).setVal(averageRaaPt610/nbinsPt610);
              hRaaNpart["Raa_pt610_AuAu62"]->bin(centBins[name]+1).setErr(sqrt(averageRaaPt610Error)/nbinsPt610);
          }
  ...
  RIVET_DECLARE_PLUGIN(PHENIX_2012_I1107625);
