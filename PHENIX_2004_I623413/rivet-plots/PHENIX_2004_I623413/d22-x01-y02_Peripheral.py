...
# Analysis object: /PHENIX_2004_I623413/d22-x01-y02_Peripheral
# Timestamp: 16-12-2025 (21:30:09)
...
ax_xLabel = '$p_{T}$ (GeV/$c$)'
ax_yLabel = '1/(2$\\pi p_{T}$) d$^{2}$N/d$p_{T}$dy ($c^{2}$/GeV$^{2}$)'
ax_title  = 'Invariant yields for $\\bar{p}$ at 60-92$\\%$ centrality'
ax_xScale = 'linear'
ax_yScale = 'linear'
xLims = (0.4, 1.8)
yLims = (0.11, 1.0)
...
labels = {
  'legend' : ([ r"Rivet [beam=AUAU130,cent=GEN]" ], r""),
}
...
exec(open(prefix+'d22-x01-y02_Peripheral__data.py').read(), dataf)
...
for label, yvals in dataf['ratio0_yvals'].items():
    ...
    if styles[label]['ratio0_histstyle']: # plot as histogram
        xpos = dataf['xedges']['Rivet.yoda [beam=AUAU130,cent=GEN]'] if styles[label]['ratio0_drawstyle'] else dataf['xpoints']['Rivet.yoda [beam=AUAU130,cent=GEN]']
    ...
    tmp = ratio0_ax.errorbar(dataf['xpoints']['Rivet.yoda [beam=AUAU130,cent=GEN]'], yvals,
    ...
    for varlabel in dataf['ratio0_variation_vals'].keys():
        if varlabel.startswith(label):
            ratio0_ax.plot(dataf['xedges']['Rivet.yoda [beam=AUAU130,cent=GEN]'],
...
plt.savefig(os.path.join(plot_outdir, 'd22-x01-y02_Peripheral.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'd22-x01-y02_Peripheral.png'), format='PNG')
