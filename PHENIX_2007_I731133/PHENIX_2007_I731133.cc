   ...
   f_corr = -b * (p_high - p_low) * pow(M_E, -b * (p_high+p_low) / 2) / (pow(M_E, -b * p_high) - pow(M_E, -b*p_low));
    ...
    /// Normalise histograms etc., after the run
      void finalize() {
          ...
          // ============================================================
// Finalize: binning + normalization + ratios (safe version)
// ============================================================

// ----------------------------
// 1) Bin-shift & normalize pp ONCE
// ----------------------------
if (sow["sow_pp"]->sumW() > 0) {

  for (const std::string key : {
        // d+Au references (Fig. 16)
        "ppEtadAuc0088",
        "ppEtadAuc0020",
        "ppEtadAuc2040",
        "ppEtadAuc4060",
        ...
        // Au+Au references (Fig. 17)
        "ppEtaAuAuc0020",
        "ppEtaAuAuc2060",
        "ppEtaAuAuc6092"
      }) {

    // One-time geometry finalization
    binShift(*hCrossSec[key]);

    // One-time pp normalization
    hCrossSec[key]->scaleW(1. / sow["sow_pp"]->sumW());
    hCrossSec[key]->scaleW(cross);
  }
}

// ----------------------------
// 2) Normalize all yield histograms ONCE
// ----------------------------
...
// ----------------------------
// 3) Ratios (NO binShift, NO rescaling inputs)
// ----------------------------

// ---------- Figure 16: RdA ----------
if (sow["sow_pp"]->sumW() > 0 && sow["sow_dAu"]->sumW() > 0) {
  divide(hEtaPt["ptyieldsdAuc0088b"],
         hCrossSec["ppEtadAuc0088"],
         hRda["EtadAuc0088"]);
  hRda["EtadAuc0088"]->scale(1. / 0.2);
}

if (sow["sow_pp"]->sumW() > 0 && sow["sow_dAuc0020"]->sumW() > 0) {
  divide(hEtaPt["ptyieldsdAuc0020b"],
         hCrossSec["ppEtadAuc0020"],
         hRda["EtadAuc0020"]);
  hRda["EtadAuc0020"]->scale(1. / 0.36);
}

if (sow["sow_pp"]->sumW() > 0 && sow["sow_dAuc2040"]->sumW() > 0) {
  divide(hEtaPt["ptyieldsdAuc2040b"],
         hCrossSec["ppEtadAuc2040"],
         hRda["EtadAuc2040"]);
  hRda["EtadAuc2040"]->scale(1. / 0.25);
}

if (sow["sow_pp"]->sumW() > 0 && sow["sow_dAuc4060"]->sumW() > 0) {
  divide(hEtaPt["ptyieldsdAuc4060b"],
         hCrossSec["ppEtadAuc4060"],
         hRda["EtadAuc4060"]);
  hRda["EtadAuc4060"]->scale(1. / 0.17);
}

if (sow["sow_pp"]->sumW() > 0 && sow["sow_dAuc6088"]->sumW() > 0) {
  divide(hEtaPt["ptyieldsdAuc6088b"],
         hCrossSec["ppEtadAuc6088"],
         hRda["EtadAuc6088"]);
  hRda["EtadAuc6088"]->scale(1. / 0.073);
}

// ---------- Figure 17: RAA ----------
if (sow["sow_pp"]->sumW() > 0 && sow["sow_AuAuc0020"]->sumW() > 0) {
  divide(hEtaPt["ptyieldsAuAuc0020b"],
         hCrossSec["ppEtaAuAuc0020"],
         hRaa["EtaAuAuc0020"]);
  hRaa["EtaAuAuc0020"]->scale(1. / 18.5);
}

if (sow["sow_pp"]->sumW() > 0 && sow["sow_AuAuc2060"]->sumW() > 0) {
  divide(hEtaPt["ptyieldsAuAuc2060b"],
         hCrossSec["ppEtaAuAuc2060"],
         hRaa["EtaAuAuc2060"]);
  hRaa["EtaAuAuc2060"]->scale(1. / 4.6);
}

if (sow["sow_pp"]->sumW() > 0 && sow["sow_AuAuc6092"]->sumW() > 0) {
  divide(hEtaPt["ptyieldsAuAuc6092b"],
         hCrossSec["ppEtaAuAuc6092"],
         hRaa["EtaAuAuc6092"]);
  hRaa["EtaAuAuc6092"]->scale(1. / 0.3);
}
