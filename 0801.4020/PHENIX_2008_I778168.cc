...
//RAA _______________________________
      ...
      book(hRaa["Raa_c3040_AuAu39"], refnameRaa5);
	  ...
	  if (collSys == AuAu)
          {
                ...
                if((c >= 0.) && (c < 5.))
                {
                    ...
                    for(const Particle& p : neutralParticles)
                    {
                        ...
                        double pt_weight = 1./(partPt*2.*M_PI); //Hmm...?
	...
	for(auto element : hPion0Pt)
      	{
        	string name = element.second->name();
          	...
          	{
              	if(element.second->numEntries() > 0) AuAu_available = true;
          	}
...
//Yields_________________
      hPion0Pt["ptyieldsc5"]->scaleW(1./sow["sow_c10"]->sumW());
...
//RAA _______________________________
      ...
      divide(hPion0Pt["c10Pt_AuAu"],hPion0Pt["c10Pt_pp"],hRaa["Raa_c010_AuAu"]);
